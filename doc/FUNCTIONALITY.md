...
| Routine-ready summary | Full-screen confirmation (S17) | User, in-app | Shown every time the auto-sorter builds a routine (onboarding finish, add/remove product); product counts + "what we arranged" (auto-adjustments) + "worth noting" (advisories); CTA to the routine |
...
7. **Routine-Ready Summary (S17):** A full-screen confirmation shown **every time the auto-sorter builds a routine** — at onboarding completion and after adding or removing a product. It reflects the sorter's actual decisions: product counts (total + per slot), a "what we arranged" list of auto-adjustments (days moved to avoid a conflict, weekly frequency reduced) and a "worth noting" list of advisories the sorter intentionally left alone (kept-together pairs). Always shown (header-only when nothing changed); single CTA to view the routine; no Undo — adjustments are edited manually in S2/S3.
