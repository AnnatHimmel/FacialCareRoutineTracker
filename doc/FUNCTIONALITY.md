...
| Manual-order indicator (S4) | Per-slot chip on Daily Home, shown only when a manual order is in effect today | Derived from active `OrderOverride` | Label carries the real count of moved products; tap opens a sheet listing those products (with their recommended position) + a "Undo the manual changes" action that reverts the override in effect (global → all days; per-day → that day). Hidden when the override matches the recommended order (nothing moved) |
...
### Daily Use
4. **Daily Home / Today's Routine (UC-8, UC-9, S4):** Primary screen. Shows today's Morning + Evening resolved routine (all daily products + occasional products scheduled today), in effective order. Users optionally record each product as "done" — reversible, never required. 6am day boundary governs which day it's credited to. When the slot's order today comes from a manual reordering (rather than the recommended order), a small per-slot **manual-changes chip** appears on the slot-header line (at the end — left in RTL) showing how many products were moved; tapping it opens a sheet that lists those products and lets the user revert to the app's automatic order. A **weekly skin-tracking reminder** card appears near the top (only when a routine exists) nudging the user to photograph and note her skin. Capture is inline — picking a photo appends it (plus any typed note) to today's skin-log entry — and the card auto-hides once any skin-log photo exists within the last 7 days (rolling cadence). The "אחר כך" (later) button dismisses it for the remainder of the day; it reappears the next day if still no photo that week.
