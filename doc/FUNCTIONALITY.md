...
| Manual-order indicator (S3, onboarding) | Per-slot chip on the order-customization screens titled "סדר המריחה בבוקר/בערב", shown only when the slot's global custom order differs from the recommended order | Derived from the slot's global `OrderOverride` | Label carries the real count of moved products; tap opens a sheet listing those products (with their recommended position) + a "ביטול השינויים הידניים" action that resets the slot to the recommended order. Replaces the former inline reset button. Hidden when the override matches the recommended order (nothing moved) |
...
| Routine-ready summary | Full-screen confirmation (S17) | User, in-app | Shown every time the auto-sorter builds a routine (mid-onboarding after category approval, add/remove product); product counts + "what we arranged" (auto-adjustments) + "worth noting" (advisories); CTA to the routine |
...
| Weekly skin-tracking reminder | Inline card at top of S4 | User, in-app | Prompts a weekly skin photo + note; inline capture writes to today's skin log; hidden once a skin-log photo exists within the last 7 days (rolling); "אחר כך" dismisses it for the rest of the day; "אל תציג שוב" turns it off permanently; only shown when a routine exists. Can be re-enabled from Settings → "תזכורת תיעוד שבועי" |
...
1. **Product Selection (UC-4, S1 — setup wizard):** Step-by-step per-category flow for first-time setup. User marks owned products per slot (Morning/Evening). Daily incompatibility conflicts surface here with mutable soft warnings. **Write-drain contract:** every toggle fires an async write chain (`upsertSelection` → `_ensureCappedSchedule` → `_resolveSlotConflicts`); its future is tracked in `_pendingWrites`. The "Next" CTA (`_V3BottomTray.onNext`) awaits `_flushWrites()` before navigating, ensuring no in-flight write races with the `buildRoutineSummary` read that immediately follows. Summary loaders (`_loadSummary` in `ProductsWizardScreen` and `OnboardingScreen`) log exceptions via `debugPrint` and retry once after a `Future.delayed(Duration.zero)` before falling back (never silently skip).
...
4. **Custom Products:** User can add their own products (name, slot, timing) via the Add Custom Product sheet, accessible from S1, S1b, and post-barcode-scan. Custom products are stored locally, not in the admin master list. They are identified by `MasterProduct.editable == true` (the `addedInVersion` field has been removed). Custom products now correctly appear in: the Order Customization "manual changes" chip, day-warning conflict/overuse detection, and receive default spread schedules via `ensureDefaultSchedules`. All custom-product reads and writes go through `RoutineService` (which exposes `watchCustomProducts`, `upsertCustomProduct`, `deleteCustomProduct` as pass-throughs).
...
6. **Order Customization (UC-6, S3):** Optional drag-to-reorder within a slot. Personal override stored locally. On the onboarding per-slot screens (titled "סדר המריחה בבוקר/בערב"), a **manual-changes chip** sits on the "general order" row, shown only when the slot's order differs from the recommended one; it carries the real count of moved products and opens a bottom sheet listing them (with their recommended position) plus a "ביטול השינויים הידניים" action that resets the slot to the recommended order (this replaces the former inline reset button).
7. **Routine-Ready Summary (S17):** A full-screen confirmation shown **every time the auto-sorter builds a routine** — mid-onboarding (right after sub-category approval, before scheduling and order customization) and after adding or removing a product. It reflects the sorter's actual decisions: product counts (total + per slot), a "what we arranged" list of auto-adjustments (days moved to avoid a conflict, weekly frequency reduced) and a "worth noting" list of advisories the sorter intentionally left alone (kept-together pairs). Always shown (header-only when nothing changed); no Undo — adjustments are edited manually in S2/S3. In the onboarding wizard the CTA label is `routineReadyReviewSlotCta(slot)` — "נתחיל עם שגרת הבוקר" for a morning-first routine, "נתחיל עם שגרת הערב" for an evening-only routine — and advances directly to the first active slot's schedule screen (not the shelf). Evening-only routines skip the AM schedule and AM order stages entirely. The onboarding wizard concludes on the Week-at-a-Glance screen in onboarding mode (no back button; CTA `weekGlanceStartGlowingCta` — "הכול מסודר, אפשר להתחיל" → `/today`). Outside onboarding the single CTA navigates to the shelf (`/collection`).

### Daily Use
4. **Daily Home / Today's Routine (UC-8, UC-9, S4):** Primary screen. Shows today's Morning + Evening resolved routine (all daily products + occasional products scheduled today), in effective order. Users optionally record each product as "done" — reversible, never required. 6am day boundary governs which day it's credited to. A **weekly skin-tracking reminder** card appears near the top (only when a routine exists) nudging the user to photograph and note her skin. Capture is inline — picking a photo appends it (plus any typed note) to today's skin-log entry — and the card auto-hides once any skin-log photo exists within the last 7 days (rolling cadence). The "אחר כך" (later) button dismisses it for the remainder of the day; it reappears the next day if still no photo that week.
...
13. **Update Review (UC-18, S14):** After a master-list version change, presents new unselected products, newly deprecated products the user has selected, and a data-intact confirmation with an export offer. New-product detection uses an **ID-snapshot mechanism**: `ReconciliationService` diffs the current master product-ID set against a persisted snapshot stored via `SettingsRepository.getKnownProductIds()/setKnownProductIds()`; `acknowledgeUpdate(version, masterProductIds)` persists the snapshot. The previous `addedInVersion` semver comparison is no longer used.
