...
- **Two roles.** The **User** experience is the app. The **Curator** authors content out of band (build/config) and issues premium license keys; *the curator authoring UI is out of scope here.* Design targets the **User** app. (PRD §2.)
...
- **Exception — curator content shown verbatim.** **Product names** and **category names** are displayed exactly as authored and are **not** translated. They frequently contain Latin-script brand names. (NFR-L2.)
...
- **Curator comments shown as written** (expected in Hebrew; may be bidi if they mention brands). The app does not translate them. (NFR-L3.)
...
| S15 | **License Activation** *(deferred, Web only)* | Enter curator-issued key to unlock cloud backup & restore | UC-21 |
...
- **Category** = curator label (e.g., Cleanser, Serum/Active, Moisturizer, SPF). Used as section headers in selection (S1). Open-ended set; tolerate arbitrary names/counts. Names shown verbatim (bidi). (UC-4, NFR-L2.)
- **Effective order** = personal override if set, else curator order; occasional items in curator position. Used wherever a routine renders (S4, S7). (UC-6, UC-8.)
- **Deprecated product** = curator-retired; usable if already selected, flagged "no longer recommended," not offered for new selection. (UC-2, UC-12.)
- **Persistent comment** = curator's note; identical everywhere the product appears; shown on expand. (UC-10.)
...
### S1 — Product Selection (UC-4, UC-7)
**Purpose:** First-run setup; also reachable later from Settings (UC-7).
**Content:** Two slots (Morning, Evening), configured independently. Within a slot, products grouped under **category headers** in curator order. Each product offers a select ("I own this") action plus enough info to identify it — name (verbatim, bidi-safe), and ideally a thumbnail / a way to peek the comment (reuse S5 expansion).
...
**Transitions:** → S2 if any selected product has a "max N/week" rule; else may go to S4.

### S2 — Schedule Setup (UC-5)
**Purpose:** For each selected occasional product, choose which weekdays to use it.
**Content:** Per product, 7 weekday toggles; **Sunday-first** to match the Sun–Sat cap week. Daily products do not appear (implicitly every day). Show the curator's cap (e.g., "Recommended: up to 3×/week").
...
**States:** default = curator order (no override); override active = indicated so reset is discoverable. Personal/local only.
...
**Purpose:** Web/iPhone users enter a **curator-issued license key** to unlock **cloud backup & on-demand restore** (not sync).
...
| **Category header** | S1 | Curator label, verbatim/bidi. (UC-4.) |
| **Weekday picker** | S2 | 7 toggles, Sunday-first; over-cap soft-warning state. (UC-5.) |
| **Soft warning (banner/inline)** | S2 (cap), S4/S5 (deprecated), S6/Web (storage) | Advisory, non-blocking; never prevents the action. (UC-5, UC-12, NFR.) |
...
6. **Order override vs. curator default** — indicate override; offer reset. (UC-6.)
...
- Curator authoring UI — out of scope for the user app. (PRD §2.)
