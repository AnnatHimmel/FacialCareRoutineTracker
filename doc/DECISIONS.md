...
### MOD-DEC-SSOT-001: Products single source of truth — RoutineService owns merged catalogue + custom CRUD
**Date**: 2026-07-01
**Request**: Custom products were invisible to several `RoutineService` methods (order chip, day warnings, default schedules) because each call site had to pass `extraProducts` manually and some forgot to. `ReconciliationService` used the `addedInVersion` semver string to detect "new" master products, a fragile approach. Screens called `userDataRepositoryProvider` directly for custom-product CRUD, bypassing the single-source-of-truth rule.
**Decision**: Six coordinated changes:
1. **`RoutineService` owns the merged product catalogue.** New `allProducts(MasterContent)` and `watchAllProducts(MasterContent)` return `[...master.products, ...customProds.map(toMasterProduct())]`. All internal derived reads (`orderForDay`, `_resolveForWeekday`, `manualOrderChangesForSlot`, `warningsForDay`, `weekGlance`, `addProduct`, `fixProblems`, `buildRoutineSummary`, `ensureDefaultSchedules`) now call `allProducts` internally. The `extraProducts` parameter is removed from every method signature and every call site. Exception: `healStaleAutoSpreadSchedules` is master-only by design (custom products have no frequency rule to heal).
2. **`RoutineService` owns custom-product CRUD.** Added `watchCustomProducts()`, `getCustomProduct(id)`, `upsertCustomProduct(p)`, `deleteCustomProduct(id)` as pass-throughs on the service. `deleteCustomProduct` preserves the existing soft-delete (sets `isDeprecated = true` and deselects slots); hard-delete only occurs via `deleteAll()` during import/replace. Screens and sheets call the service, not `userDataRepositoryProvider`, for custom products.
3. **`allProductsProvider`** (`StreamProvider<List<MasterProduct>>`) is added to `root_providers.dart` as the canonical merged product stream for every screen. `customProductsProvider` is updated to delegate to `routineServiceProvider.watchCustomProducts()`.
4. **`MasterProduct.editable`** is a new stored `bool` (default `false`). Custom products carry `editable = true`. This is the single UI indicator for "user-authored product". All code that previously branched on `addedInVersion == 'custom'` now reads `editable`.
5. **`addedInVersion` field removed** from `MasterProduct`, the cache serializer (`SharedPrefsMasterContentCache`), and `UserCustomProduct.toMasterProduct()`. The Supabase column/RPC drop is staged in `supabase/05_drop_added_in_version.sql` for the next release; in-the-wild v8 clients still parse the key, so the live RPC is not altered yet. The bundled JSON retains the now-ignored key until release-time regen.
6. **`ReconciliationService` new-product detection** now diffs the current master product-ID set against a persisted ID snapshot (`SettingsRepository.getKnownProductIds()/setKnownProductIds()`). `acknowledgeUpdate(version, masterProductIds)` persists the snapshot. The `addedInVersion` semver comparison is gone.
**Bugs fixed as a result**: custom products now correctly appear in the Order Customization "manual changes" chip, in day-warning conflict/overuse detection, and receive default spread schedules via `ensureDefaultSchedules`.
**Rationale**: Passing `extraProducts` at every call site was error-prone (MOD-DEC-SUM-002 had to add the parameter; several other methods still lacked it). Moving the merge into the service makes it impossible to forget. The `addedInVersion` field was never a reliable "new product" signal (it is a content-version string, not a boolean flag); an explicit ID snapshot is both clearer and cheaper to evaluate.
**Alternatives Rejected**: Merging custom products into `MasterContent` at load time (couples two independent data domains — master content is admin-authored and bundled; custom products are user data); keeping `extraProducts` with a lint rule (lint rules are opt-in; the service contract is enforced at compile time); hard-deleting `addedInVersion` from Supabase immediately (in-the-wild v8 clients would crash parsing the RPC response if the column vanishes).
**Affects Files**: `lib/domain/entities/master_product.dart` (`editable` added, `addedInVersion` removed), `lib/domain/services/routine_service.dart` (`allProducts`/`watchAllProducts`, CRUD pass-throughs, `extraProducts` removal), `lib/domain/services/reconciliation_service.dart` (ID-snapshot detection), `lib/data/cache/shared_prefs_master_content_cache.dart` (`addedInVersion` removed from serializer), `lib/shared/providers/root_providers.dart` (`allProductsProvider`, `customProductsProvider` delegation), all screens/sheets previously calling `userDataRepositoryProvider` for custom products, `supabase/05_drop_added_in_version.sql` (new, staged), `test/domain/services/routine_service_test.dart`, `test/data/cache/shared_prefs_master_content_cache_test.dart`, `doc/ARCHITECTURE.md`, `doc/DECISIONS.md`, `doc/FUNCTIONALITY.md`, `CLAUDE.md`.

### MOD-DEC-MASTER-SYNC-001: Resync stale bundle from Supabase, self-heal stale schedules, custom products on Order screen
**Date**: 2026-06-30
**Request**: User reported (a) the sub-category dropdown empty for every product, (b) prod-007 (a sunscreen) stuck at "3×/week" though Supabase says daily, and (c) the Order screen showing fewer products than Week Glance. Also asked whether the bundled JSON is worth keeping given recurring drift.
**Decision**: Four-part fix, all on `main` (user-authorized), bundle kept as the offline-first fallback but treated as a **generated artifact**:
1. **Regenerate the bundle** via the existing `scripts/sync_from_supabase.dart` (Supabase-wins-per-id). A full Supabase-vs-bundle diff confirmed the drift was: `subCategoryId` missing on all 33 products + prod-007 morning `weeklyMax 3`→`daily` (the only config drift) + brand/ingredients/barcodes enrichment + `imageAsset` flipped to Supabase Storage URLs. The URL switch is **not** an offline regression: `ProductThumb`/`product_detail_screen` map a remote URL's basename to `assets/images/products/<basename>` as the offline placeholder, and all 33 basenames have a matching local asset (verified). **Script fix:** `_mergeById` previously replaced matched items wholesale, which silently dropped the bundle-local subcategory `keywords` array (no Supabase column) and broke `ProductClassifier`. Changed it to merge per-key (`{...local, ...remote}`): Supabase wins each shared key, local-only keys (`keywords`) survive. The sync is idempotent (second run = no diff). To be wired into `release-prep` with a `git diff --exit-code assets/data/` guard so it can't silently rot.
2. **Cache key bump** `master_content_cache_v1`→`v2` (`SharedPrefsMasterContentCache`), purging the legacy key on write/clear. Needed because the existing cache guard only discards on `contentVersion` decrease, and the version stayed `1.1.0` while schema content (subcategories) changed — so a stale null-subcategory cache would otherwise survive.
3. **Self-heal migration** `RoutineService.healStaleAutoSpreadSchedules({master})`: for any **selected** product whose **current** master rule is `DailyRule` and whose stored `WeekdaySchedule` row equals exactly `spreadN7(n)` for its own size, promote it to all-7. Fixes prod-007 (a stale `spreadN7(3)={0,2,4}` row that, per `effectiveDays`, otherwise wins over the corrected daily rule). Empty (suppressed), all-7, and hand-picked rows are left untouched; custom products are out of scope. Run once via `startupMigrationProvider` gated on the previously-unused user schema version (`<2` → run → set 2), watched fire-and-forget in `SkincareApp`.
4. **Order screen custom products**: `OrderCustomizationScreen` now watches `customProductsProvider` and `_sortedProducts` iterates `[...master.products, ...custom]`, matching every other routine surface. Persisted overrides already key by product-id string, so dragging a custom product works with no schema change.
**Rationale**: Drift is a process problem, not a reason to drop offline support; regenerating + automating the existing script is the single-source-of-truth fix. The schedule cap is user-data that the content fix alone can't reach (explicit row wins), so a guarded one-time heal is required; the `spreadN7`-exact-match heuristic only ever clears machine-generated rows.
**Accepted tradeoff**: a user who *deliberately* picked days that happen to equal a spread (must include day 0, e.g. Sun/Tue/Thu) on a now-daily product is reset to all-7 by the one-time pass. Judged acceptable (runs once; daily products only; exact-spread only).
**Alternatives Rejected**: deleting the bundle and fetching only at runtime (breaks first-launch-offline + offline-first guarantee); hand-editing the JSON (reintroduces drift); deleting the schedule row via a new repo method (would break ~30 `UserDataRepository` test fakes — upserting all-7 is effect-equivalent for a daily product and needs no interface change); adding a bespoke guard flag to `SettingsRepository` (the user schema version already exists and was unused).
**Affects Files**: `scripts/sync_from_supabase.dart` (per-key merge), `.claude/skills/release-prep/SKILL.md` (Phase 0 sync + drift guard), `assets/data/master_products.json` (regenerated), `lib/data/cache/shared_prefs_master_content_cache.dart`, `lib/domain/services/routine_service.dart`, `lib/shared/providers/root_providers.dart`, `lib/app.dart`, `lib/features/setup/order_customization_screen.dart`, `test/data/cache/shared_prefs_master_content_cache_test.dart`, `test/domain/services/routine_service_test.dart`, `test/features/setup/order_customization_screen_test.dart`, `doc/ARCHITECTURE.md`, `doc/DECISIONS.md`.

### MOD-DEC-MANUAL-ORDER-002: Relocate manual-order chip to the order screens (supersedes -001's placement)
**Date**: 2026-06-27
**Request**: Move the manual-changes indication off Daily Home and onto the screens that control product ordering — the onboarding per-slot order screens titled "סדר המריחה בבוקר/בערב" — using the same bottom-sheet popup. (User decisions: onboarding order screens only; the chip+popup **replaces** the existing "איפוס לסדר המומלץ" reset.)
**Decision**: Removed the Daily Home wiring and its day-filtered read. Replaced `RoutineService.manualOrderChanges({slot, weekday})` + `revertEffectiveOrder` with `manualOrderChangesForSlot({master, slot})`, which sorts the **full selected slot set** (not day-filtered — matching the order screen's reorderable list) two ways: by the slot's **global** `OrderOverride` and by `ProductSorter.adminComparator`, reporting products whose index differs (`MovedProduct.targetPosition` = recommended position). Revert is a plain `deleteOrderOverride(slot)`. New `slotManualOrderChangesProvider(Slot)` (master products only — the order screen does not list custom products) feeds a `ManualOrderChip` placed on the "general order" row of the onboarding `_OnboardingOrderHeader` layout, pushed to the end (left in RTL); a single leading info icon (the reorder-arrows icon was dropped). The chip opens `showManualOrderSheet(slot, onReverted:)`; `onReverted` clears the screen's `_localOrder[slot]` so the list visually resets. The advanced-options panel's global reset button was removed (replaced by the chip+popup); the per-day reorder section and the standard (Settings) order screen's own reset are unchanged.
**Rationale**: The order screen is where ordering is actually controlled, so the indicator + revert belong there; on Daily Home it duplicated information the user can't act on in context. Full-slot global scope matches exactly what that screen displays and edits, so the count is truthful. Still a pure derived read over the existing `OrderOverride` row, routed through `RoutineService` per §3.0.
**Alternatives Rejected**: Keeping it on Daily Home (user reversed that); also adding it to the Settings edit-order screen (out of scope per user — onboarding screens only); keeping the day-filtered computation (would undercount vs. the full slot list shown on the order screen).
**Affects Files**: `lib/domain/services/routine_service.dart`, `lib/shared/providers/root_providers.dart`, `lib/features/setup/order_customization_screen.dart`, `lib/features/home/daily_home_screen.dart` (reverted), `lib/shared/widgets/manual_order_sheet.dart`, `lib/shared/widgets/manual_order_chip.dart`, `test/domain/services/routine_service_test.dart`, `doc/ARCHITECTURE.md`, `doc/FUNCTIONALITY.md`, `doc/UI_DESIGN.md`.

### MOD-DEC-MANUAL-ORDER-001: Daily Home manual-order chip + revert sheet (superseded by -002)
**Date**: 2026-06-27
**Request**: On Daily Home, indicate per slot (Morning/Evening separately) when the routine order is a manual override, show only when changes exist, and let the user open a sheet listing the changed products and revert to the app's automatic order. Counts and products must be real data.
**Decision**: Added two derived members to `RoutineService` (the single source of truth for routine reads/writes): `manualOrderChanges({master, slot, weekday})` → `ManualOrderChanges {hasOverride, isGlobalScope, weekday, moved: List<MovedProduct{product, targetPosition}>}`, and `revertEffectiveOrder({slot, weekday})`. `manualOrderChanges` resolves the slot for the day twice — once with the effective override (`getEffectiveOrderOverride`), once with the order it would fall back to on revert (a per-day override falls back to the global order; a global override falls back to the admin/recommended order) — and reports the products whose index differs, tagged with their 1-based position in the post-revert order. `revertEffectiveOrder` deletes exactly the one override in effect: the per-day row if one exists for that weekday, otherwise the global row. A `manualOrderChangesProvider((date, slot))` (resolving against master **+ custom** products, like `dailyRoutineProvider`) feeds a slot-tinted outlined `ManualOrderChip` inserted between the slot header and the product list, shown only when `moved` is non-empty. The chip opens `showManualOrderSheet`, which lists each moved product (thumbnail + name + brand + recommended-position badge) and a "ביטול השינויים הידניים" button.
**Revert scope (confirmed with user)**: revert removes whichever override is *in effect* — if today's order comes from the general (all-days) order, every day returns to automatic; if from a day-specific order, only that day reverts. Chosen over a "today-only" revert (which would require synthesising a per-day "pinned automatic" row) as the literal, predictable meaning of "remove that override".
**Rationale**: Keeps the indicator a pure read over existing `OrderOverride` rows — no new persisted state, no new schema. Deriving `moved` against the *post-revert* order (not always admin) makes the chip honestly preview what revert will do. Routing the read + write through `RoutineService` honours the §3.0 single-source-of-truth rule.
**Alternatives Rejected**: Storing a separate "is-manual" flag (redundant with `OrderOverride`); diffing only against the admin order (would misreport for a per-day override layered on a global one); computing the diff in the widget/provider (bypasses the scheduler contract); today-only revert (added complexity, can itself look like a manual change).
**Affects Files**: `lib/domain/services/routine_service.dart`, `lib/shared/providers/root_providers.dart`, `lib/shared/widgets/manual_order_chip.dart` (new), `lib/shared/widgets/manual_order_sheet.dart` (new), `lib/features/home/daily_home_screen.dart`, `lib/core/l10n/app_en.arb`, `lib/core/l10n/app_he.arb` (+ generated), `test/domain/services/routine_service_test.dart`, `test/shared/widgets/manual_order_chip_test.dart` (new), `doc/ARCHITECTURE.md`, `doc/FUNCTIONALITY.md`, `doc/UI_DESIGN.md`.
...
**Affects Files**: `routine_service.dart`, new `routine_build_summary.dart`, new `routine_ready_summary_screen.dart`, `onboarding_screen.dart`, `add_product_flow_screen.dart`, `add_custom_product_sheet.dart`, `app_he.arb`/`app_en.arb` (8 `routineReady*` keys), and tests.
...
**Affects Files**: `lib/features/setup/product_selection_screen.dart`, `lib/domain/services/routine_service.dart`, `test/playwright/tests/schedule.spec.ts` (new e2e regression test).

### MOD-DEC-SUM-002: Custom Products Invisible to RoutineService after Add-Product Flow
**Date**: 2026-06-24
...
**Decision**: Three methods in `RoutineService` now accept `List<MasterProduct> extraProducts = const []` and build a combined list `[...master.products, ...extraProducts]`:
...
**Affects Files**: `lib/domain/services/routine_service.dart`, `lib/shared/providers/root_providers.dart`, `lib/features/setup/add_product_flow_screen.dart`, `pubspec.yaml`.

**Superseded 2026-07-01** by MOD-DEC-SSOT-001: `extraProducts` removed; `RoutineService` now fetches custom products internally via `allProducts(MasterContent)`. Per-call-site `extraProducts` passing is no longer necessary or supported.
