...
### Routine Data Access — `RoutineService` is the single source of truth
All **routine** device data — `ProductSelection`, `WeekdaySchedule`, `OrderOverride` — is read and written **only** through `RoutineService` (`lib/domain/services/routine_service.dart`), exposed as `routineServiceProvider`. **Never** call `userDataRepositoryProvider` directly for these three tables from a screen or provider; use the scheduler's `watch*` streams and mutation methods (`addProduct`/`removeProduct`/`setDays`/`toggleDay`/`removeDay`/`setOrder`/`resetOrder`/`fixProblems`/…). The scheduler also owns the derived reads (`orderForDay`, `warningsForDay`, `weekGlance`) by composing `RoutineResolver`, `WeekGlanceBuilder`, `IncompatibilityChecker`, `ConflictResolver`, `ProductSorter`.

- **`RoutineService` also owns the merged product catalogue** (`allProducts(MasterContent)` / `watchAllProducts(MasterContent)`). All derived reads fetch custom products internally — the `extraProducts` parameter has been removed from every method. Use `allProductsProvider` (`StreamProvider<List<MasterProduct>>` in `root_providers.dart`) as the single merged product read for every screen. Do not assemble `[...master, ...custom]` in screens.
- **`RoutineService` also owns custom-product CRUD** (`watchCustomProducts()`, `getCustomProduct`, `upsertCustomProduct`, `deleteCustomProduct`). Never call `userDataRepositoryProvider` for custom products from a screen or sheet.
- **`MasterProduct.editable`** (`bool`, default `false`) is the only distinction between admin master products and user-authored custom products. The `addedInVersion` field has been removed from `MasterProduct` and the cache serializer. The matching Supabase column/RPC drop is staged in `supabase/05_drop_added_in_version.sql` for the next release (deferred while v8 clients are live).
...
- Need a new routine read/write? **Extend `RoutineService`** (TDD), then consume it — do not bypass it. Full rationale: `doc/ARCHITECTURE.md` §3.0.
...
All Flutter chevron `IconData` values (`Icons.chevron_left`, `Icons.chevron_right`, and their `_rounded`/`_outlined`/`_sharp` variants) have `matchTextDirection: true`. In an RTL layout this causes the icon to be **automatically mirrored** — `chevron_left` renders as `>` and `chevron_right` renders as `<`.
...
For trailing navigation indicators (list rows, cards, banners), the correct icon depends on ambient text direction:

```dart
// Direction-aware trailing nav chevron — correct in both RTL and LTR
Icon(
  Directionality.of(context) == TextDirection.rtl
      ? Icons.chevron_left        // < points left — correct trailing indicator in RTL
      : Icons.chevron_right,      // > points right — correct trailing indicator in LTR
...
Never use a bare `Icons.chevron_left` or `Icons.chevron_right` on a trailing indicator without this direction check — it will be wrong in one of the two locales.
